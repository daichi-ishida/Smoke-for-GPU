...
void Laplacian(const ScalarField xField, const Obstacles obstacles, ScalarField AxField)
{
    ...
    if (!obstacles.indexSampler(index))
    {
        ...
        Ax = xField(top) + xField(left) + xField(right) + xField(bottom) - 4.0f * xField(index);
    }
...
__global__
void applyPressureGradient_k(const ScalarField pressureField, const Obstacles obstacles, uField u, vField v, float dt)
{
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    int y = threadIdx.y + blockIdx.y * blockDim.y;
    int offset = x + y * xRes;
    ...
    {
        if (!obstacles.indexSampler(index)
            && !obstacles.indexSampler(index.right()))
        {
            float old_u = u(x + 1, y);
            u(x + 1, y) = old_u - dt * (pressureField(x + 1, y) - pressureField(x, y)) / (RHO * DX);
        }
        else
        {
            u(x + 1, y) = 0.0f;
    ...
    if (y < yRes - 1)
    {
        ...
        {
            float old_v = v(x, y + 1);
            v(x, y + 1) = old_v - dt * (pressureField(x, y + 1) - pressureField(x, y)) / (RHO * DX);
        }
        else
        {
            v(x, y + 1) = 0.0f;
...
void Simulator::cg()
{
    ...
    CALL_KERNEL(applyPressureGradient_k, blocks, threads)(m_data->pressure, m_data->obstacles, m_data->u0, m_data->v0, m_data->dt);
}
